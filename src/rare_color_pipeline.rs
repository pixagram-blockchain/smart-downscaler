...
fn rare_color_survives_full_pipeline() {
    ...
    let config = DownscaleConfig {
        ...
        region_weight: 0.24,
        k_centroid: 4,             // NEW: salient tile color
        k_centroid_iterations: 2,
        color_rarity: 0.35,        // NEW
        detail_boost: 0.9,         // NEW (exercises compute_saliency_map)
        reserve_colors: 3,         // NEW (exercises reservation)
        ..Default::default()
    };
    ...
    assert!(min_de < 0.03, "lip color missing from palette (min ΔE={min_de:.4})");
