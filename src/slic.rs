...
impl Default for SlicConfig {
    fn default() -> Self {
        Self { num_superpixels: 100, compactness: 10.0, max_iterations: 10, convergence_threshold: 1.0 }
    }
}

#[derive(Clone, Copy, Debug)]
struct ClusterCenter { lab: Lab, x: f32, y: f32 }
...
impl Segmentation {
    pub fn get_label(&self, x: usize, y: usize) -> usize {
        if x < self.width && y < self.height { self.labels[y * self.width + x] } else { 0 }
    }
    ...
    pub fn get_segment_pixels(&self, segment_id: usize) -> Vec<(usize, usize)> {
        ...
        for y in 0..self.height {
            for x in 0..self.width {
                if self.labels[y * self.width + x] == segment_id { pixels.push((x, y)); }
            }
        }
        pixels
    }
    ...
    pub fn get_segment_neighbors(&self, segment_id: usize) -> Vec<usize> {
        ...
        for y in 0..self.height {
            for x in 0..self.width {
                if self.labels[y * self.width + x] != segment_id { continue; }
                let mut check = |nx: i32, ny: i32| {
                    if nx >= 0 && ny >= 0 && (nx as usize) < self.width && (ny as usize) < self.height {
                        let nl = self.labels[ny as usize * self.width + nx as usize];
                        if nl != segment_id { neighbors.insert(nl); }
                    }
                };
                check(x as i32 - 1, y as i32);
                check(x as i32 + 1, y as i32);
                check(x as i32, y as i32 - 1);
                check(x as i32, y as i32 + 1);
            }
        }
    ...
    pub fn merge_small_segments(&mut self, min_size: usize, pixels: &[Rgb]) {
        ...
        loop {
            ...
            for seg_id in 0..self.num_segments {
                if self.segment_sizes[seg_id] == 0 || self.segment_sizes[seg_id] >= min_size { continue; }
                ...
                let best_neighbor = neighbors.iter()
                    .min_by(|&&a, &&b| {
                        seg_color.distance_squared(self.segment_colors[a])
                            .partial_cmp(&seg_color.distance_squared(self.segment_colors[b])).unwrap()
                    }).copied();
                if let Some(merge_into) = best_neighbor {
                    for label in self.labels.iter_mut() { if *label == seg_id { *label = merge_into; } }
                    ...
                    for (i, &label) in self.labels.iter().enumerate() {
                        if label == merge_into { sum = sum + labs[i]; count += 1; }
                    }
                    if count > 0 { self.segment_colors[merge_into] = sum / count as f32; }
                    merged_any = true;
                }
            }
            if !merged_any { break; }
    ...
    fn compact_labels(&mut self) {
        ...
        for &label in &self.labels {
            if !label_map.contains_key(&label) { label_map.insert(label, next_label); next_label += 1; }
        }
        for label in self.labels.iter_mut() { *label = label_map[label]; }
...
pub fn slic_segment(pixels: &[Rgb], width: usize, height: usize, config: &SlicConfig) -> Segmentation {
    let labs: Vec<Lab> = pixels.iter().map(|p| p.to_lab()).collect();
    ...
    while y < height {
        ...
        while x < width {
            centers.push(ClusterCenter { lab: labs[y * width + x], x: x as f32, y: y as f32 });
            x += grid_step;
        }
        y += grid_step;
    }
    if centers.is_empty() {
        centers.push(ClusterCenter { lab: labs[0], x: (width / 2) as f32, y: (height / 2) as f32 });
    }

    // Move centers to lowest gradient
        ...
        for dy in -1..=1 {
            for dx in -1..=1 {
                ...
                if nx >= 0 && ny >= 0 && (nx as usize) < width && (ny as usize) < height {
                    ...
                    if g < min_grad { min_grad = g; best_x = nx as f32; best_y = ny as f32; }
                }
            }
        }
        center.x = best_x;
        center.y = best_y;
        center.lab = labs[best_y as usize * width + best_x as usize];
    }
    ...
    for _iter in 0..config.max_iterations {
        ...
        for (k, center) in centers.iter().enumerate() {
            ...
            for y in y_min..=y_max {
                for x in x_min..=x_max {
                    let idx = y * width + x;
                    let dc = center.lab.distance(labs[idx]);
                    let dx = x as f32 - center.x;
                    let dy = y as f32 - center.y;
                    let ds = (dx * dx + dy * dy).sqrt();
                    let d = dc + (m / s) * ds;
                    if d < distances[idx] { distances[idx] = d; labels[idx] = k; }
                }
            }
        }

        let mut new_centers = vec![(Lab::new(0.0,0.0,0.0), 0.0f32, 0.0f32, 0usize); num_centers];
        for y in 0..height {
            for x in 0..width {
                let idx = y * width + x;
                let k = labels[idx];
        ...
        for (k, center) in centers.iter_mut().enumerate() {
            ...
            if count > 0 {
                ...
                let (new_x, new_y) = (sum_x / n, sum_y / n);
        ...
        if max_shift < config.convergence_threshold { break; }
    }
    ...
    for &label in &labels { if label < num_centers { segment_sizes[label] += 1; } }

    Segmentation { width, height, labels, num_segments: num_centers, segment_colors, segment_centers, segment_sizes }
}
...
fn compute_gradient(labs: &[Lab], width: usize, height: usize) -> Vec<f32> {
    ...
    for y in 1..height.saturating_sub(1) {
        for x in 1..width.saturating_sub(1) {
            let idx = y * width + x;
            let gx = labs[idx + 1].distance_squared(labs[idx - 1]);
            let gy = labs[idx + width].distance_squared(labs[idx - width]);
            gradient[idx] = gx + gy;
        }
    }
    gradient
    ...
    for start_y in 0..height {
        for start_x in 0..width {
            ...
            while let Some((x, y)) = queue.pop() {
                let idx = y * width + x;
                if visited[idx] || labels[idx] != original_label { continue; }
                visited[idx] = true;
                component.push(idx);
            ...
            for &idx in &component { new_labels[idx] = current_label; }
            current_label += 1;
        }
    }
...
pub fn flood_fill_segment(pixels: &[Rgb], width: usize, height: usize, color_threshold: f32) -> Segmentation {
    let labs: Vec<Lab> = pixels.iter().map(|p| p.to_lab()).collect();
    ...
    for start_y in 0..height {
        for start_x in 0..width {
            let start_idx = start_y * width + start_x;
            if labels[start_idx] != usize::MAX { continue; }
            ...
            while let Some((x, y)) = queue.pop() {
                let idx = y * width + x;
                if labels[idx] != usize::MAX { continue; }
                if seed_color.distance(labs[idx]) > color_threshold { continue; }
                labels[idx] = current_label;
                sum_color = sum_color + labs[idx];
                sum_x += x as f32;
                sum_y += y as f32;
                count += 1;
                if x > 0 { queue.push((x - 1, y)); }
    ...
    Segmentation { width, height, labels, num_segments: current_label, segment_colors, segment_centers, segment_sizes }
}
...
mod tests {
    ...
    fn test_slic_basic() {
        ...
        for y in 0..10 {
            for x in 0..10 {
                let color = if x < 5 && y < 5 { Rgb::new(255,0,0) }
                else if x >= 5 && y < 5 { Rgb::new(0,255,0) }
                else if x < 5 && y >= 5 { Rgb::new(0,0,255) }
                else { Rgb::new(255,255,0) };
                pixels.push(color);
            }
        }
        let config = SlicConfig { num_superpixels: 4, compactness: 10.0, max_iterations: 5, convergence_threshold: 1.0 };
    ...
    fn test_flood_fill() {
        ...
        for _y in 0..10 {
            for x in 0..10 {
                pixels.push(if x < 5 { Rgb::new(255,0,0) } else { Rgb::new(0,0,255) });
            }
        }
